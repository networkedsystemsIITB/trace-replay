...
main (int argc, char *argv[])
{
  ...
  cmd.AddValue ("R", "Random variable", R);
  cmd.AddValue ("nWifi", "Number of client", nWifi);
  cmd.Parse (argc, argv);

  long double stopTime = 1000;
  ...
  for (uint32_t i = 0; i < nWifi; i++)
    {
      ...
      // application.SetTraceFile (traceFilePath);
      application.SetRandomStream (i);
      application.SetStopTime (Seconds (stopTime));
      application.SetPortNumber (49153 + 200 * i);
      // application.SetStartTimeOffset (Seconds (10.0));
