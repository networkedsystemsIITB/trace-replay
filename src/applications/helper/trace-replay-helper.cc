...
void
TraceReplayHelper::SetRandomStream (int64_t stream)
{
  NS_LOG_FUNCTION (this);
...
double
TraceReplayHelper::CalPacketDelay (uint32_t frameNum, bool timeOut, bool httpReq, double currTime, double packetTime)
{
  double delay = 0;
...
TraceReplayHelper::ProcessPacket (m_connId id, uint32_t packetSize, double packetTime, uint32_t frameNum)
{
  ...
  double delay = CalPacketDelay (frameNum, m_timeoutMap[frameNum], m_httpReqMap[frameNum],
  ...
  if (packet.GetDelay () > 0)
  {
    ...
    for (it = m_connMap.begin (); it != m_connMap.end (); it++)
    {
      ...
      {
        packet.AddParallelConn ((it->first).portClient, (it->first).portServer, (it->second).totByteCount);
      }
    }
  }
      ...
      for (uint32_t i = 0; i < numPacket; i++)
        {
          ...
          file << packet.GetSize () << "\t" << packet.GetDelay () << std::endl;
          if (packet.GetDelay () > 0)
            {
              uint32_t numParallelCon = packet.GetNumParalleConn ();
              file << numParallelCon << std::endl;
              for (uint32_t j = 0; j < numParallelCon; j++)
                {
                  std::pair<uint16_t, uint16_t> connId = packet.GetConnId (j);
  ...
  for (uint32_t j = 0; j < numConn; j++)
    {
      ...
      // Client to server connection
      {
        ...
        }
        {
                  ...
                  for (uint32_t i = 0; i < n; i++)
                    {
                      ...
                      packet.AddParallelConn (srcPort, dstPort, count);
                    }
                }
          ...
          client->SetConnId (ipClient, portClient, ipServer, portServer);
      ...
      // Server to client connection
      {
        ...
        }
        {
          ...
          server->SetConnId (ipClient, portClient, ipServer, portServer);
