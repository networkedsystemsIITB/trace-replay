...
uint32_t
TraceReplayPacket::GetByteCount (uint16_t portClient, uint16_t portServer)
{
...
uint32_t
TraceReplayPacket::GetNumParalleConn ()
{
  return m_parallelConnList.size ();
}

uint32_t
TraceReplayPacket::GetByteCount (uint32_t i)
{
  return m_parallelConnList[i].m_byteCount;
}

std::pair<uint16_t, uint16_t>
TraceReplayPacket::GetConnId (uint32_t i)
{
  return std::make_pair (m_parallelConnList[i].m_srcPort, m_parallelConnList[i].m_dstPort);
}

void
TraceReplayPacket::AddParallelConn (uint16_t srcPort, uint16_t dstPort, uint32_t count)
{
...
double
TraceReplayPacket::GetDelay ()
{
  return m_delay.GetSeconds ();
}

uint32_t
TraceReplayPacket::GetSize ()
{
  return m_size;
}
